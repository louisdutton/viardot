export class Voice {   
    ...
    constructor() {
        ...
        this.bufferDuration = this.bufferSize/sampleRate;
        this.context.audioWorklet.addModule('voice-processor').then(() => {
            this.glottis = new AudioWorkletNode(context, 'glottis-processor');
            this.tract = new AudioWorkletNode(context, 'tract-processor');
        });

        this.synth = this.context.create(this.bufferSize, 2, 1);
    ...
    start()
    {
        ...
        var aspirationNoise = this.context.createBufferSource();
        aspirationNoise.buffer = this.generateWhiteNoiseNode(2*sampleRate);
        aspirationNoise.loop = true;
        
        var aspirationFilter = this.context.createBiquadFilter();
        aspirationFilter.type = "bandpass";
        aspirationFilter.frequency.value = 500;
        aspirationFilter.Q.value = 0.5;
        aspirationNoise.connect(aspirationFilter).connect(this.synth);
        
        var fricativeFilter = this.context.createBiquadFilter();
        fricativeFilter.type = "bandpass";
        fricativeFilter.frequency.value = 1000;
        fricativeFilter.Q.value = 0.5;
        aspirationNoise.connect(fricativeFilter).connect(this.synth);;      
        
        aspirationNoise.start(0);
    }
    
    generateWhiteNoise(frameCount)
    {
        var buffer = this.context.createBuffer(1, frameCount, sampleRate);
        var channel = myArrayBuffer.getChannelData(0);
        for (var i = 0; i < frameCount; i++) { channel[i] = Math.random(); }
        return buffer;
    }
    ...
    mute() { this.synth.disconnect(); }
    unmute() { this.synth.connect(this.context.destination); }
}
